...
async function pageStudy(req, res) {
    ...
    const query = `
        ...
        WHERE EXISTS (
            ...
            AND class_scheduke.time_from <= ${timeToMinutes}
            AND class_schedule.time_to > ${timeToMinutes}
        )
        AND classes.subject = '${filters.subject}'
    `

    // casi haja erro na hora da consulta do banco de dados
    try {
...
async function saveClasses (req, res) {
    const createProffu = require("./database/createProffy")
    ...
    const classScheduleValues = req.body.whatsapp.map((weekday, index) => {
        return {
            weekday,
    ...
    try {
        const db = await Database
        await createProffu(db, { proffyValue, classValue, classScheduleValues })    
